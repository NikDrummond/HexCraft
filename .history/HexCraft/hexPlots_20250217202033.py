...
def hex_Patches(
    hexs: Hexagons, s: float = 1, method: str = "flat_side", **kwargs
) -> mc.PolyCollection:
...
def get_ax_limits(hexs: Hexagons, s: float = 1, padding: float = 1.0, method: str) -> jnp.ndarray:
    coords = hex_2D_conversion(hexs, s=s, method="flat_side")
    arr = jnp.array(
        [
