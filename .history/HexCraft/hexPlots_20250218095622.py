...
def hex_Patches(
    hexs: Hexagons, size: float = 1, scale: float = 1.0, method: str = "flat_side", **kwargs
) -> mc.PolyCollection:
    coords = hex_2D_conversion(hexs, s=size, method=method)
    if method == 'flat_top':
        hexagons = vmap(_flat_top_verts, in_axes=(0, None))(coords, size, scale)
    elif method == 'flat_side':
        hexagons = vmap(_point_top_verts, in_axes=(0, None))(coords, size, scale)
