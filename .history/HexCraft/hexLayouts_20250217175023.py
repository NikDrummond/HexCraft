...
def _point_flat_top(a: jnp.ndarray, s: float = 1.0) -> jnp.ndarray:
    _flat_top = jnp.array([[3 / 2, 1], [jnp.sqrt(3) / 2, jnp.sqrt(3)]])
    return s * _mat_mul(a, _flat_top)


@jit
def _point_pointy_top(a: jnp.ndarray, s: float = 1.0) -> jnp.ndarray:
    _pointy_top = jnp.array([[jnp.sqrt(3), jnp.sqrt(3) / 2], [1, 3 / 2]])
    return s * _mat_mul(a, _pointy_top)
