...
def test_length():
    ...
    assert hexMath.hex_length == 2, 'Not calculating distance from origin properly'

def test_dist():
    h1 = Hexagon(jnp.array([2,-2,0]))
    h2 = Hexagon(jnp.array([2,0,-2]))
    assert hexMath.hex_distance(h1,h2) == 2, 'Not calulating distance between two hexagons properly'
